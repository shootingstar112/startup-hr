...
function Row({
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3">
      ...
      <div className={`text-sm font-black text-white ${valueClassName ?? ""}`}>
        {value}
      </div>
    </div>
  );
...
export default function SeveranceCalculator() {
  ...
  const [last3mStart, setLast3mStart] = useState(() => {
    const ee = new Date(defaultEnd + "T00:00:00");
    const ls = addMonths(ee, -3); // 시작: 퇴사일 기준 3개월 전
    return isoDate(ls);
  });
  ...
  const taxBarSeverance =
    result.severanceGross > 0
      ? (result.severanceNet / result.severanceGross) * 100
      : 0;
        ...
        <div className="space-y-5">
          <div className="rounded-xl border p-4 overflow-hidden">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-600">
              ...
              <span className="rounded-full bg-slate-50 px-2 py-1">
                근속연수(일/365):{" "}
                <span className="font-black text-slate-900">
                  {Number.isFinite(result.serviceYears)
                    ? result.serviceYears.toFixed(4)
                    : "0.0000"}
                </span>
              </span>
            </div>
          ...
          <div className="rounded-xl border p-4 overflow-hidden">
            ...
            <div className="mt-3 rounded-xl bg-slate-50 p-3 text-xs font-semibold text-slate-700">
              최근 3개월 임금총액(반영 후):{" "}
              <span className="font-black text-slate-900">
                {formatWon(result.last3mWagesTotal)}
              </span>
              <div className="mt-1 text-[11px] text-slate-600">
                · 상여금 반영액(3개월분): {formatWon(result.bonusCountedIn3m)} /
                연차수당 반영액: {formatWon(result.leavePayCountedIn3m)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-slate-900 p-6 text-white shadow-sm">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-white/10 p-4">
              ...
              <div className="mt-1 text-lg font-black">
                {formatWon(result.avgYearSeveranceGross)}
              </div>
            </div>
            <div className="rounded-2xl bg-white/10 p-4">
              <div className="text-xs font-extrabold text-white/70">
          ...
          <div className="mt-5">
            <div className="mb-2 flex items-center justify-between text-xs font-bold text-white/80">
              <span>실수령 비율</span>
              <span>
                실수령 {(taxBarSeverance || 0).toFixed(1)}% · 세금{" "}
                {(taxBarTax || 0).toFixed(1)}%
              </span>
            </div>
            <div className="h-3 w-full overflow-hidden rounded-full bg-white/15">
              <div
                ...
                style={{
                  width: `${Math.max(0, Math.min(100, taxBarSeverance))}%`,
                }}
              />
            </div>
