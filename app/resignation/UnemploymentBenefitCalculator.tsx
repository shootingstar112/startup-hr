...
export default function UnemploymentBenefitCalculator() {
  ...
  const computed = useMemo(() => {
    ...
    // 4) B(하한기반)
    const floorHourly = MIN_WAGE_2026 * FLOOR_RATE;
          ...
          <div className="rounded-xl border p-4 overflow-hidden">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {preset.map((p) => {
                ...
                return <Chip key={p.label} label={p.label} active={active} onClick={() => setInsuredMonthsText(String(p.months))} />;
              })}
            </div>

            <div className="mt-3 text-xs font-semibold text-slate-600">
              예상 소정급여일수: <span className="font-black text-slate-900">{computed.payableDays}일</span>
            </div>
          </div>
        </div>
            ...
            <div className="divide-y divide-white/10">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <div className="text-sm font-black text-white">
                  {computed.payableDays}일{isDisabled ? " (장애인)" : computed.age >= 50 ? " (만50세이상)" : ""}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-2xl bg-white/10 p-4">
            ...
            <div className="mt-3 space-y-3 text-sm font-semibold text-white/85">
              <div>
                ...
                <div className="text-white/75">입력한 평균임금(1일) ≈ {formatWonPlain(Math.round(computed.avgDailyWage))}원</div>
              ...
              <div>
                ...
                <div className="text-white/75">최저시급×80% = {formatWonPlain(Math.round(computed.floorHourly))}원/시간</div>
                <div className="text-white/75">인정시간 = min({computed.dailyHoursRaw}h, 8h) = {computed.인정시간}h</div>
                <div className="text-white/75">
